...
### Added
- `ring deployment exec` runs a command, or opens an interactive shell, inside a running instance (Docker runtime; other runtimes report that they do not support it). Opt-in via `[server.exec] enabled = true`, and a server with it off answers `501`. Full terminal behaviour: raw mode, live resize, and the command's exit code becomes the CLI's, so `ring deployment exec app --no-tty -- test -f /ready && …` composes like a local command. Sessions are bounded by a concurrency limit and by duration and idle timeouts, because a container runtime offers no way to kill an exec once started: past those limits Ring stops relaying and frees the slot. Exec reaches only instances Ring manages (the container's ownership label is re-checked before the session opens), and an exec stream ticket cannot be replayed against logs, or the reverse. Requires an `admin` token rather than `deployments:write`, since a shell reads every mounted secret, and each opened session is written to the namespace audit trail
