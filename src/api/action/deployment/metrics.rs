        ...
        Ok(Some(deployment)) => {
            if deployment.runtime != "docker" {
                return (
                    ...
                    Json(json!({ "error": "metrics are only available for docker deployments" })),
                )
                    .into_response();
            }
            ...
            match Runtime::new(deployment.clone()) {
                Ok(runtime) => {
                    ...
                    let total_cpu: f64 =
                        container_stats.iter().map(|c| c.cpu_usage_percent).sum();
                    ...
                    let total_rx: u64 =
                        container_stats.iter().map(|c| c.network.rx_bytes).sum();
                    let total_tx: u64 =
                        container_stats.iter().map(|c| c.network.tx_bytes).sum();
                    ...
                    let total_read: u64 =
                        container_stats.iter().map(|c| c.disk_io.read_bytes).sum();
                    let total_write: u64 =
                        container_stats.iter().map(|c| c.disk_io.write_bytes).sum();
