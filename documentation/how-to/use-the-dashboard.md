...
Boots a tiny web server on your machine that serves the static UI and reverse-proxies `/api/*` to the API URL of your current context. Your bearer token (from `auth.json` or `RING_TOKEN`) is injected on the way out, so the browser never sees it.
