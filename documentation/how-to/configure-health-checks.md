...
**Proxy bonus:** a `readiness: true` check of `type: command` is **also** translated into a native Docker `HEALTHCHECK`. Traefik / Sozune-style proxies that gate on `Status: healthy` won't route traffic to the new container while the readiness command fails. See [Health checks design → proxy integration](/documentation/concepts/health-checks-design#proxy-integration).
