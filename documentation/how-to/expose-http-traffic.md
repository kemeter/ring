...
- Per-deployment routing via labels — no central config file to edit on every change
...
> **Docker socket warning.** Mounting `/var/run/docker.sock` gives Sozune full read access to the host's Docker daemon. Treat the Sozune container as privileged. Mount read-only (`permission: ro`) — Sozune only reads container metadata, it doesn't need to start or stop containers.
...
The simplest setup keeps Sozune **in the same namespace** as its backends — they share the `ring_<namespace>` bridge automatically. If you need a single Sozune fronting multiple namespaces, attach Sozune's container to each backend network:
...
- **One Sozune per host port.** Two Sozune deployments can't both publish `:80` — pick one as the edge.
